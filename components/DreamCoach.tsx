...
const DreamCoach: React.FC = () => {
  ...
  const [limitStatus, setLimitStatus] = useState(checkLimit('chat'));
  ...
  useEffect(() => {
    ...
    const initializeCoach = async () => {
        ...
        const chatSession = ai.chats.create({
          ...
          config: {
            systemInstruction: `You are a dedicated AI Dream Coach and Symbolism Expert named 'Murabbiy'. You speak in ${language}. Answer only about dreams.`,
          },
        });
        setChat(chatSession);
  ...
  const handleSendMessage = async (text: string) => {
    ...
    const currentLimit = checkLimit('chat');
    if (!currentLimit.canUse) {
        alert(translations.limitReached || "Limit reached!");
        return;
    }
    ...
    try {
      ...
      incrementUsage('chat');
      setLimitStatus(checkLimit('chat'));
    } catch (error) {
      console.error(error);
  ...
  return (
    <div className="max-w-4xl mx-auto flex flex-col h-full max-h-[85vh]">
      <div className="text-center mb-6 relative">
        ...
        <div className="flex justify-center mt-2">
            <span className="text-[10px] font-bold text-cyan-400 uppercase tracking-widest bg-cyan-900/20 px-3 py-1 rounded-full border border-cyan-500/20">
                Xabarlar: {limitStatus.remaining} / 15
            </span>
        </div>
      ...
      <div ref={chatContainerRef} className="flex-grow bg-gray-900/40 border border-white/10 rounded-3xl p-6 space-y-6 overflow-y-auto mb-4 custom-scrollbar backdrop-blur-sm shadow-inner relative">
        {!limitStatus.canUse && (
      ...
      <form onSubmit={handleFormSubmit} className="flex items-center gap-3 bg-gray-900/60 p-2 rounded-full border border-white/10 backdrop-blur-md">
        <textarea
          ...
          rows={1}
          disabled={isLoading || isInitializing || !limitStatus.canUse}
        />
        <button type="submit" disabled={isLoading || isInitializing || !userInput.trim() || !limitStatus.canUse} className="w-10 h-10 flex items-center justify-center text-white bg-gradient-to-r from-purple-600 to-cyan-500 rounded-full shadow-lg hover:shadow-cyan-500/50 transform enabled:hover:scale-105 transition-all duration-300 disabled:opacity-50 disabled:cursor-not-allowed">
